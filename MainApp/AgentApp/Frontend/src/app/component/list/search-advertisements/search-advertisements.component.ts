...
export class SearchAdvertisementsComponent implements OnInit {
  ...
  getPicture(adFromSearch: AdvertisementWithPictures) {
    this.carService.getPicture(adFromSearch.car.pictures[0].data, adFromSearch.car.id).subscribe(
      (data) => {
  ...
  createImageFromBlob(image: Blob, adFromSearch: AdvertisementWithPictures) {
