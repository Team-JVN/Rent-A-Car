...
export class AddCarComponent implements OnInit {
  ...
  ngOnInit() {
    this.carForm = this.formBuilder.group({
      ...
      mileageInKm: new FormControl(null, [
        Validators.required,
        Validators.min(0),
      ]),
      kidsSeats: new FormControl(null, [
        Validators.required,
        Validators.min(0),
        Validators.max(3),
      ]),
  ...
  create() {
    ...
    if (this.files.length == 0 || this.files.length > 5) {
      this.toastr.error(
        "You have to choose at least one picture. Maximum is 5.",
        "Create Car"
      );
      return;
    }
    ...
    const car = new Car(
      ...
      this.carForm.value.fuelType,
      this.carForm.value.gearBoxType,
      this.carForm.value.bodyStyle,
      this.carForm.value.mileageInKm,
      this.carForm.value.kidsSeats,
      this.carForm.value.availableTracking
    );
  ...
  onSelect(event: any) {
    if (event.rejectedFiles.length > 0) {
      this.toastr.warning(
        "Maximum picture size is 2MB. Pictures larger than that are being automatically discarded.",
        "Add pictures"
      );
    }
